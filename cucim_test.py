...
def run_cucim(
    ...
    cuimg = CuImage(svs_path)

    print(cuimg.size())
    # CuImage.size -> (width, height)
    h, w = int(cuimg.size()[0]), int(cuimg.size()[1])

    timings = {}

    # L0 전체 로드: read_region 으로 한 번에 로드 (주의: 메모리 매우 큼)
    full_img_np = None
    if not skip_full_load:
        t0 = time.perf_counter()
        # cuCIM read_region 은 (x, y), (w, h), level=0
        full_region = cuimg.read_region(location=(0, 0), size=(w, h), level=0)
        # numpy 로 호스트 메모리로 복사
        full_img_np = np.asarray(full_region)
    ...
    for (x, y) in coords:
        if full_img_np is not None:
            ...
            region = cuimg.read_region(location=(x, y), size=(patch, patch), level=0, as_cuda=True)
            patch_arr = np.asarray(region)  # host로
